...
% ---------------------------------------------------------------
\titulo{Otimização Inteligente de Rotas no Senac Considerando Qualidade de Wi-Fi: Uma Comparação entre Algoritmo de Grafos e Inteligência Artificial} %Trocar titulo
\autor{
...
O tema surge ao observarmos a baixa conectividade de Wi-Fi no campus do Senac Santo Amaro e imaginar uma aplicação
 com algoritmos de grafos que otimize a transição entre pontos do senac, garantindo uma boa conectividade com 
 Wi-Fi em todo o trajeto
A ideia é ultilizar um algoritmo de grafos otimizado para encontrar a menor rota entre dois pontos, tendo como peso
das arestas a conexão Wi-Fi e a distância entre os vertices.
Após uma coleta de dados, será treinado um modelo de Inteligência Artificial, que fará previsões da melhor 
rota possível, dado um ponto de origem e um ponto de destino.
Por fim e não menos importante, será realizada uma comparação entre o algoritmo de grafos otimizado e a 
Inteligência Artificial, com base nos seguintes indicadores:\newline
- Passos/metros \newline
- dBm (decibel-miliwatt)\newline
- Número de pontos com Wi-Fi fraco \newline
(Conta quantos pontos da rota têm sinal abaixo de um limite que será definido).\newline
- Tempo de execução dos dois métodos (Segundos)\newline
- Variância do sinal Wi-Fi.\newline
- Similaridade entre rotas (Mede quanto a rota da IA se aproxima da rota ótima do algoritmo em porcentagem).\newline
...
\begin{itemize}
    \item \textbf{Relevância acadêmica}: O fato de que otimização de rotas é um problema resolvido de modo geral
    com algoritmos de grafo, permite explorar uma nova abordagem pouco implementeda, que é a utilização de frequência
    de Wi-Fi como peso para a decisão de ir ao próximo vertice e a ultilização de IA treinada para decidir a melhor rota,
    sem a dependência um algoritmo, usado convencionalmente quando tratamos de otimização de rotas.

    \item \textbf{Relevância prática/profissional}: A solução proposta traz benefícios diretos para usuários e 
    instituições ao permitir a identificação e utilização de rotas com melhor qualidade de conexão Wi-Fi dentro 
    do campus. Para os usuários, isso possibilita uma experiência mais estável ao acessar conteúdos online, 
    participar de aulas remotas, utilizar sistemas acadêmicos e realizar atividades que dependem de conectividade 
    contínua.\newline

    Para a instituição, a ferramenta pode servir como apoio na gestão da infraestrutura de rede, 
    auxiliando na identificação de áreas com baixa cobertura e orientando investimentos em melhorias. 
    Além disso, a aplicação pode contribuir para a otimização do uso dos espaços físicos, promovendo 
    maior eficiência no deslocamento de alunos e colaboradores.
